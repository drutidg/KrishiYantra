    ...
    <div className="bg-white rounded-3xl p-4 border border-slate-200/90 shadow-soft hover:shadow-card transition-all">
      ...
      {status === 'HIGH_DEMAND' && (
        <div className="p-2 bg-rose-50 rounded-xl border border-rose-200/90 mb-2.5 text-[11px] text-rose-950 font-medium">
          <span>🔥 High Demand Alert: Heavy congestion (~{estimatedWaitMins}m wait, {currentQueue} farmers). Consider nearby centers to save time.</span>
        </div>
      )}

      {isBusy && (
        <div className="p-2 bg-amber-50 rounded-xl border border-amber-200/80 mb-2.5 text-[11px] text-amber-900 font-medium">
          <span>⚠️ High Demand: Operational with longer wait (~{estimatedWaitMins}m wait, {currentQueue} farmers in queue).</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          ...
          <span>{isClosed ? 'Closed' : availableSlotsCount === 0 ? 'Center Full' : 'Book Slot'}</span>
        </Link>
      </div>
    </div>
  );
