...
export default function BookingWizardPage({
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // Default fallback demo farmer
      setFarmerUser({ id: 'demo-ravi', name: 'Ravi Kumar', phone: '9876543210' });
    }

    async function loadCenterAndSlots() {
      try {
        setLoading(true);
        const res = await fetch(`/api/centers/${params.centerId}`);
        const data = await res.json();
        if (data.success) {
          setCenter(data.center);
          setSlots(data.center.slots || []);

          // Preselect slot from query param if available
          const preSlotId = searchParams.get('slotId');
          if (preSlotId) {
            const match = data.center.slots?.find((s: any) => s.id === preSlotId);
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({
          farmerId: farmerUser?.id || 'demo-ravi',
          ...
          <div className="space-y-3 p-4 bg-emerald-50/70 rounded-2xl border border-emerald-200 text-sm">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-emerald-200/60">
              ...
              <span className="font-bold text-emerald-800">
                ~{center?.estimatedWaitMins || 25} minutes
              </span>
            </div>
