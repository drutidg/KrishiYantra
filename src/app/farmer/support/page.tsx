...
export default function FarmerHelpCenterPage() {
  ...
  useEffect(() => {
    ...
    fetchTickets(stored?.id || 'demo-ravi');
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div id="tickets" className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => fetchTickets(user?.id || 'demo-ravi')}
      ...
      <SupportTicketModal
        ...
        user={user}
        onTicketCreated={() => fetchTickets(user?.id || 'demo-ravi')}
      />
      ...
      <TicketDetailsModal
        ...
        onMessageSent={() => fetchTickets(user?.id || 'demo-ravi')}
      />
    </div>
  );
}
