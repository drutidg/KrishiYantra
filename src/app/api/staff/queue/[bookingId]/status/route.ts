  ...
  try {
    ...
    if (status === 'PROCESSING') {
      await notificationService.send({
        ...
        message: `Token ${booking.tokenNumber}: Your procurement is now being processed at Counter #1.`,
        type: 'QUEUE',
      });
      ...
      if (currentlyProcessing === 0) {
        ...
        if (nextWaiting) {
          ...
          await notificationService.send({
            ...
            message: `Token ${nextWaiting.tokenNumber}: Please proceed immediately to the procurement counter.`,
            type: 'QUEUE',
          });
      ...
      await notificationService.send({
        ...
        message: `Token ${booking.tokenNumber}: Your turn is approaching. Please remain near the procurement hall.`,
        type: 'QUEUE',
      });
    }
    ...
    waitingQueue.slice(0, 3).forEach(async (b, idx) => {
      ...
      if (pos === 3) {
        await notificationService.send({
          ...
          bookingId: b.id,
          title: 'Turn Approaching',
          message: `Token ${b.tokenNumber}: Your turn is approaching. You are 3 positions away.`,
          type: 'QUEUE',
        });
      } else if (pos === 1) {
        await notificationService.send({
