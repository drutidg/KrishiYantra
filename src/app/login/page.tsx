      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl items-center">
        <section className="grid w-full overflow-hidden rounded-[2rem] border border-emerald-950/10 bg-white shadow-2xl lg:grid-cols-[0.85fr_1.15fr]">
          <div className="bg-[#124734] p-8 text-white sm:p-12">
            ...
            <p className="mt-6 max-w-sm text-sm leading-7 text-emerald-50/75">Farmers manage visits and tokens. Vendors manage procurement operations and queues.</p>
          </div>
          <div className="p-6 sm:p-12">
            ...
            <div className="grid gap-4">
              ...
              <Link href="/vendor/login" className="group rounded-2xl border border-slate-200 p-5 transition hover:border-emerald-500 hover:bg-emerald-50"><div className="flex items-start justify-between"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-amber-100 text-amber-800"><Building2 className="h-6 w-6" /></span><ArrowUpRight className="h-5 w-5 text-slate-300 transition group-hover:text-emerald-700" /></div><h3 className="mt-6 text-xl font-black">Vendor portal</h3><p className="mt-2 text-sm leading-6 text-slate-500">Sign in or register a vendor account to operate procurement center workflows.</p></Link>
            </div>
            <p className="mt-8 text-center text-xs text-slate-400">Existing staff demo access remains available at <Link className="font-bold text-emerald-700" href="/staff/login">Staff Desk</Link>.</p>
          </div>
        </section>
      </div>
    </main>
